"use strict";
/**
 * NayePankh Backend Server v2
 * ───────────────────────────
 * Architecture:
 *   Express → Routes (volunteer, chat) → Utils (chatEngine, mlRunner) → MongoDB
 *
 * Chatbot tiers:
 *   1. OpenAI GPT-3.5  (if OPENAI_API_KEY is valid)
 *   2. Enhanced rule-based engine with session memory (always available)
 *
 * ML Pipeline:
 *   TF-IDF + Logistic Regression (86.5% CV accuracy, 3 classes)
 *   Outputs: role + confidence score + per-class probabilities
 */
...
// Backward-compat alias: POST /predict → POST /chat/predict
...
async function connectDB() {
  if (!config.mongoUri) {
    logger.warn("[db] MONGO_URI not set — skipping database connection");
    return;
  }
  try {
    ...
    // Don't crash — chatbot still works without DB
  }
}
...
async function start() {
  ...
  app.listen(config.port, () => {
    logger.info(`[server] NayePankh backend v2 running on port ${config.port}`);
    logger.info(`[server] OpenAI: ${config.openaiKey ? "configured ✅" : "not configured — using local fallback"}`);
    logger.info(`[server] ML model dir: ${config.mlModelDir}`);
  });
}
